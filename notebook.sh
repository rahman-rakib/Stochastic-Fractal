...
if [ $? -eq 0 ]; then
    ...
    if [ -f "$ROOT_PATH/software/anaconda3/etc/profile.d/conda.sh" ]; then
        . "$ROOT_PATH/software/anaconda3/etc/profile.d/conda.sh"
    else
        export PATH="$ROOT_PATH/software/anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
